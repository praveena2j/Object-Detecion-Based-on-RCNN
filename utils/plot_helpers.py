...
def plot_test_set_results(evaluator, num_images_to_plot, results_base_path, cfg):
	...
	with open(cfg["DATA"].TEST_MAP_FILE) as f:
		content = f.readlines()
	img_base_path = os.path.dirname(os.path.abspath(cfg["DATA"].TEST_MAP_FILE_NO_ROI))
	...
	for i in range(0, num_images_to_plot):
		...
		for i in range(len(counter)):
			if(i>0):
				print(str(classes[i]) + ': ' + str(counter[i]))

		for (box, label, score) in zip(filtered_bboxes, filtered_labels, filtered_scores):
			if(label>0):
				print(str(classes[label]) + ': ' + str(score))
				print('Box ' + str(classes[label]) + ':x1=' + str(box[0]) + ',y1=' + str(box[1]) + ',x2=' + str(box[2]) + ',y2=' + str(box[3]))
				#print(box, classes[label], score)
